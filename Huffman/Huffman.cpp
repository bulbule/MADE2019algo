...
// num of leaves + symbol + 00..0 + 1 + code + ... + encoded data + used bits in last byte
// so the total code for each symbol is 0..0 + 1 + code from tree, where the total size
...
void Huffman::save_decoded_data() {
	...
	for (int i = 0; i < decoded_data.size(); ++i) {
		...
		for (int j = 0; j < path.size(); j++) {
			if (path[j] == '0') current_node = current_node->left;

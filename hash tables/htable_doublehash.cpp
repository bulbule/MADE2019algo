...
class HashTable {
    public:
     HashTable(): buffer(default_size, Node()),  \
    ...
    // and an odd h2(k)
     int hash1(const std::string& key, size_t container_size) const {
        // coeff1 must be coprime with table size
        const int coeff1 = 11;
        int hash = 0;
        size_t size = key.size();
        for (int i = 0; i < size; ++i) {
            hash = (hash * coeff1 + key[i]) % container_size;
        }
        return hash;
    }

     int hash2(const std::string& key, size_t container_size) const {
        // coeff2 must be coprime with table size
        const int coeff2 = 131;
        int hash = 0;
        size_t size = key.size();
        for (int i = 0; i < size; ++i) {
            hash = (hash * coeff2 + key[i]) % container_size;
        }
...
void HashTable::rehash() {
    ...
    std::vector<Node> new_buffer(new_buffer_size, Node());
    for (int i = 0; i < buffer_size; ++i) {
        if (!buffer[i].key.empty() && !buffer[i].deleted) {
            // get new hash for the new table
            int h1 = hash1(buffer[i].key, new_buffer_size);
            int h2 = hash2(buffer[i].key, new_buffer_size);
            int count = 0;
            while (!new_buffer[h1].key.empty() && count < new_buffer_size) {
...
bool HashTable::has(const std::string& key) const {
    ...
    int count = 0;
    while (!buffer[h1].key.empty() && count < buffer_size) {
...
bool HashTable::add(const std::string& key) {
    ...
    if (deleted_key_ind != -1) {
        buffer[deleted_key_ind] = Node(key);
    } else {
        buffer[h1] = Node(key);
    }
    num_of_nodes++;
